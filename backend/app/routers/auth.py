...
async def register(payload: RegisterRequest):
    ...
    token = auth_service.create_jwt(str(user.id), user.role, user.email)
...
async def google_auth_callback(code: str = Query(...)):
    ...
    if not user:
        user = User(
            email=email,
            password_hash=auth_service.hash_password("google_oauth_default_pwd"),
            full_name=name,
            role=Role.PATIENT,
    ...
    jwt_token = auth_service.create_jwt(str(user.id), user.role, user.email)
    return RedirectResponse(url=f"{settings.FRONTEND_URL}/login?token={jwt_token}&role={user.role.value}")